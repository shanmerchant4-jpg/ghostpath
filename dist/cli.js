...
import { openProject, stopProject, listProjects, findProject, readGhostfile, } from './ghost/orchestrator.js';
...
// ── Helpers ──────────────────────────────────────────────────────────────────
function asciiBar(percent, width = 40) {
...
program
    ...
    try {
        ...
        const header = chalk.bold.white('Name'.padEnd(W.name)) +
        ...
        for (const p of projects) {
            ...
            console.log(p.name.slice(0, W.name - 1).padEnd(W.name) +
...
program
    ...
    console.log(`RAM  ${ramColor(asciiBar(ramPercent))} ${ramColor(`${ramUsedMB} MB / ${ramTotalMB} MB`)}`);
    console.log();
    process.exit(0);
});
...
program
    ...
    const tick = setInterval(() => {
        ...
        console.log(chalk.dim(`Updated: ${new Date().toLocaleTimeString()}  |  Press Ctrl+C to stop\n`));
