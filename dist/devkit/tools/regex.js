...
function parseAtom(pattern, pos) {
    ...
    if (ch === '\\') {
        ...
        if (next === undefined)
            return { desc: 'a literal backslash', length: 1 };
        let desc;
        switch (next) {
            case 'd':
                desc = 'a digit (0–9)';
            ...
            case 'D':
                ...
                break;
            case 'w':
                desc = 'a word character (a–z, A–Z, 0–9, _)';
                break;
            case 'W':
                desc = 'a non-word character';
                break;
            case 's':
                desc = 'a whitespace character';
                break;
            case 'S':
                desc = 'a non-whitespace character';
                break;
            case 'b':
                desc = 'a word boundary';
                break;
            case 'B':
                desc = 'a non-word boundary';
                break;
            case 'n':
                desc = 'a newline';
                break;
    ...
    if (ch === '^')
        return { desc: 'start of string', length: 1 };
    if (ch === '$')
        return { desc: 'end of string', length: 1 };
    if (ch === '.')
        return { desc: 'any character except newline', length: 1 };
    if (ch === '[') {
        ...
        while (end < pattern.length) {
            if (pattern[end] === ']' && end !== contentStart)
                break;
            if (pattern[end] === '\\')
            ...
            switch (pattern[pos + 2]) {
                ...
                case '<':
                    if (pattern[pos + 3] === '=') {
                        ...
                        groupDesc = 'named capturing group';
                        scanFrom = pos + 4;
                    }
                    break;
            }
    ...
    if (special.includes(ch))
        return { desc: `"${ch}"`, length: 1 };
    return { desc: `the character "${ch}"`, length: 1 };
}
function readQuantifier(pattern, pos) {
    const ch = pattern[pos];
    if (ch === undefined)
        return null;
    ...
    if (ch === '*')
        return { prefix: 'zero or more' + lazySuffix, length: isLazy ? 2 : 1 };
    if (ch === '+')
        return { prefix: 'one or more' + lazySuffix, length: isLazy ? 2 : 1 };
    if (ch === '?')
        return { prefix: 'optionally' + lazySuffix, length: isLazy ? 2 : 1 };
    if (ch === '{') {
