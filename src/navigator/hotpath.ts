...
export function getHotPaths(graph: CallGraph, topN: number): CallNode[] {
  return [...graph.nodes.values()]
    .sort((a, b) => b.callCount - a.callCount)
    .slice(0, topN);
}

export function getSlowPaths(graph: CallGraph, thresholdMs: number): CallNode[] {
  return [...graph.nodes.values()]
...
export function formatHotPaths(nodes: CallNode[]): string {
  if (nodes.length === 0) {
    return chalk.dim('No hot paths recorded yet.');
  }

  const header =
    chalk.bold.white('#'.padEnd(COL.rank)) +
