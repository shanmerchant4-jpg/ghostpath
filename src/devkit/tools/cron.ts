...
function parseValue(raw: string, nameMap?: Record<string, number>): number {
  const lower = raw.toLowerCase();
  if (nameMap !== undefined && lower in nameMap) return nameMap[lower]!;
...
function parseCron(expression: string): ParsedCron {
  ...
  const [minuteF, hourF, dayF, monthF, weekdayF] = parts as [
    string, string, string, string, string,
  ];

  const weekdays = expandField(weekdayF, 0, 7, WEEKDAY_NAMES);
  // Cron treats 7 as Sunday (same as 0)
  if (weekdays.has(7)) { weekdays.add(0); weekdays.delete(7); }
...
function formatTime(minuteF: string, hourF: string): string {
  const step = (f: string, prefix: string): string | null => {
    const m = f.match(/^\*\/(\d+)$/);
  ...
  if (minuteStep && hourF === '*') return minuteStep;
  if (minuteF === '0' && hourStep) return hourStep;
  if (minuteF === '0' && hourF === '*') return 'every hour, on the hour';
  if (minuteF === '*' && hourF !== '*') {
  ...
  if (!isNaN(h) && !isNaN(m)) {
    ...
    if (h < 12) return `at ${h}:${mm} AM`;
    if (h === 12) return `at 12:${mm} PM`;
...
function formatDayPart(dayF: string, monthF: string, weekdayF: string): string | null {
  if (dayF === '*' && monthF === '*' && weekdayF === '*') return null;
