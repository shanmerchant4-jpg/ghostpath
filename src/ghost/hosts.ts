...
function entryLine(domain: string): string {
...
async function readHostsFile(): Promise<string> {
  try {
    ...
    throw new GhostError({
      ...
      message: `Could not read ${HOSTS_FILE}: ${(err as Error).message}`,
      hint:
        code === 'EACCES'
...
async function writeHostsFile(content: string): Promise<void> {
  try {
    ...
    throw new GhostError({
      ...
      message: `Could not write to ${HOSTS_FILE}: ${(err as Error).message}`,
      hint:
        code === 'EACCES'
          ? 'Try running with elevated permissions (sudo on Linux/macOS)'
...
function confirmWithUser(message: string): Promise<boolean> {
...
export async function hasHostEntry(domain: string): Promise<boolean> {
...
export async function addHostEntry(domain: string): Promise<void> {
  ...
  const confirmed = await confirmWithUser(
    `GhostPath needs to add "${domain}" to ${HOSTS_FILE}. Allow?`,
  );
