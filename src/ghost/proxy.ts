...
export async function startProxy(domain: string, port: number): Promise<void> {
...
export async function stopProxy(domain: string): Promise<void> {
  if (!routeMap.has(domain)) {
    throw new GhostError({
...
export function getProxyPort(domain: string): number | undefined {
